...
public:
    ...
    inline bool isShared() const { return !chunk.isDetached(); }
    void detach();
    QByteArray toByteArray();

    // getters
    inline int head() const { return headOffset; }
    inline int size() const { return tailOffset - headOffset; }
    inline int capacity() const { return chunk.size(); }
    inline int available() const { return chunk.size() - tailOffset; }
    inline const char *data() const { return chunk.constData() + headOffset; }
    inline char *data()
    {
        if (isShared())
            detach();
    ...
    inline void reset() { headOffset = tailOffset = 0; }
    inline void clear() { assign(QByteArray()); }
...
public:
    ...
    inline bool isEmpty() const { return bufferSize == 0; }
    ...
    inline qint64 size() const { return bufferSize; }
